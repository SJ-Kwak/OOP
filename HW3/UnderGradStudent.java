...
public class UnderGradStudent implements Student {
	...
	double[][]score=new double[8][6];//이차원 배열. 행: 학기, 열: 과목
	double scoreavg[]=new double[8];//학기평균 저장 배열.
	int semester;//학기수 저장 변수
	double avgtotal=0;//총평균 저장 변수
	
	public UnderGradStudent(String name) {//name을 매개변수로 하는 생성자
		this.name=name;
	}
	...
	public void addSemester(int i, double s[]) {//학기별 과목 점수 기록 메소드
		...
		for(int j=0;j<6;j++) {//6과목 점수 합산
			score[semester][j]=s[j];//이차원 배열의 '열'에 메인클래스에서 받은 과목 점수 저장
			scoresum+=score[semester][j];//과목 점수 합산
		}
		scoreavg[semester-1]=scoresum/6;//학기별 과목 점수 평균 저장. semester는 학기 변수라 1부터 시작하기 때문에 배열에의 사용을 위해 semester-1을 해준다.
	...
	public void showScore() {//학기별 점수, 평균 출력 메소드
		for(int m=1;m<=semester;m++) {//학기 출력
			...
			for(int n=0;n<6;n++) {//학기별 과목 점수 출력
